...
class PointerTable extends List {
  constructor (input) {
    ...
    this.word = new UInt('word');
  }
  decode (rom) {
    const list = [];
    const seen = {};

    var count = this.wrap ? this.word.decode(rom) : Infinity;

    while (!super.ended(list)) {
      catcher(`PointerTable(${list.length})`, () => {
        let offset = list.length > count ? this.wrap : this.offset;
        let pointer = this.word.decode(rom) + offset;
  ...
  encode (list, rom) {
    ...
    for (let i = 0; i < list.length; i++) {
      catcher(`PointerTable(${i})`, () => {
        ...
        this.word.encode(seen[key] - offset, rom);
